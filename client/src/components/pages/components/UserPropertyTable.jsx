...
const UserPropertyTable = () => {
  ...
  const deleteProperty = async (id) => {
    try {
      const response = await axios.delete(`${import.meta.env.VITE_PROPERTY_API}/delete/${id}`,{withCredentials: true});
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-fit">
                      <div className="flex flex-col gap-6 items-start">
                        <Button variant="link" className="cursor-pointer" updateid={uProperty._id}>
                          <View />
                          View
                        </Button>
                        <Button variant="link" className="cursor-pointer text-red-500" onClick={() => deleteProperty(uProperty._id)}>
                          <Delete />
                          Delete
                        </Button>
                      </div>
